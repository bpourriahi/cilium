...
func runServe(vp *viper.Viper) error {
	...
	if vp.GetBool(keyTLSClientDisabled) {
		...
		tlsClientConfig, err := certloader.NewWatchedClientConfig(logger,
	...
	if vp.GetBool(keyTLSServerDisabled) {
		...
		tlsServerConfig, err := certloader.NewWatchedServerConfig(logger,
		...
		if err != nil {
			return err
		} else if !tlsServerConfig.KeypairConfigured() {
			return errors.New("no TLS certificate or private key provided for the server")
		}
